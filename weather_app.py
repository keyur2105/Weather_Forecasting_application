...
# User input
st.markdown("### Location and Temperature")
location = st.selectbox("Location", df['Location'].unique())
Mintemp = st.number_input("Minimum Temperature (°C)", format="%.2f")
Maxtemp = st.number_input("Maximum Temperature (°C)", format="%.2f")

st.markdown("### Rainfall and Wind")
Rainfall = st.number_input("Rainfall (mm)", format="%.2f")
windGustDire = st.selectbox("Wind Gust Direction", df['WindGustDir'].unique())
windGustSpeed = st.number_input("Wind Gust Speed (km/h)", format="%.2f")
windDir9am = st.selectbox("Wind Direction at 9:00 AM", df['WindDir9am'].unique())
windDir3pm = st.selectbox("Wind Direction at 3:00 PM", df['WindDir3pm'].unique())
windspeed9am = st.number_input("Wind Speed at 9:00 AM (km/h)", format="%.2f")
windspeed3pm = st.number_input("Wind Speed at 3:00 PM (km/h)", format="%.2f")

st.markdown("### Humidity and Pressure")
humidity9am = st.number_input("Humidity at 9:00 AM (%)", format="%.2f")
humidity3pm = st.number_input("Humidity at 3:00 PM (%)", format="%.2f")
pressure9am = st.number_input("Pressure at 9:00 AM (hPa)", format="%.2f")
pressure3pm = st.number_input("Pressure at 3:00 PM (hPa)", format="%.2f")

st.markdown("### Rain Today")
raintoday = st.selectbox("Rain Today", ['Yes', 'No'])

if st.button("Predict"):
    # Validate inputs
    if Mintemp >= Maxtemp:
        st.error("Minimum Temperature should be less than Maximum Temperature.")
    elif windspeed9am > windGustSpeed or windspeed3pm > windGustSpeed:
        st.error("Wind Speed at 9:00 AM and 3:00 PM should be less than Wind Gust Speed.")
    else:
        ...
        try:
            if (location and Mintemp and Maxtemp and Rainfall and windGustDire and windGustSpeed and 
                ...
                if predict == 'YES':
                    st.markdown("### 🌧️ Prediction: Rain will come tomorrow.")
                else:
                    st.markdown("### ☀️ Prediction: No rain tomorrow.")
